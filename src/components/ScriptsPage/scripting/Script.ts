...
// COMMENT parameters are ignored in the script execution but displayed in the UI
...
export type ScriptParameter =
    | { name: string, type: "COMMENT"; value: boolean }
...
// CONSTRUCT_RGB creates a COLOR_RGB variable from three NUMBER values or variables
// CONDITION works on BOOLEAN (condition) and NUMBER (then, else) values and produces a new NUMBER variable
// CONDITION_RGB works on BOOLEAN (condition) and COLOR_RGB (then, else) values and produces a new COLOR_RGB variable
...
type ScriptComputation =
    | { name: string, type: "CONSTRUCT_RGB"; red: number | ScriptVariable; green: number | ScriptVariable; blue: number | ScriptVariable }
    | { name: string, type: "CONDITION"; condition: ScriptVariable; then: number | ScriptVariable; else: number | ScriptVariable }
    | { name: string, type: "CONDITION_RGB"; condition: ScriptVariable; then: ScriptVariable; else: ScriptVariable }
...
export type ScriptAction =
    ...
    | { type: "K_SET_CONTINUOUS"; fixture: string; program: string; parameterName: string; value: number | ScriptVariable }
    | { type: "K_SET_DISCRETE"; fixture: string; program: string; parameterName: string; value: string };
